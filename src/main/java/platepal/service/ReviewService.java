 ...
 *
 * Empty and over-long text is validated by the Review model, while the
 * one-review-per-restaurant rule and the "only your own" rule need the
 * repository and the session, so they live here.
 *
 * This service does not touch the rating repository.
 */
public class ReviewService {
     ...
     * @throws IllegalArgumentException if no such restaurant exists, the
     *                                  restaurant has not been visited, or the
    ...
    public Review writeReview(String restaurantId, String content) {
        ...
        if (!user.hasVisited(id)) {
            throw new IllegalArgumentException(
                    "You can only review restaurants in your Visited list. "
                            + "Mark \"" + id + "\" as visited first.");
        }

        Optional<Review> existing =
                reviewRepository.findByUserAndRestaurant(
                        user.getId(),
                        id);
    ...
    public Optional<Review> getMyReview(String restaurantId) {
        ...
        return reviewRepository.findByUserAndRestaurant(
                user.getId(),
                restaurantId);
    }
    ...
    public boolean removeMyReview(String restaurantId) {
        ...
        existing.ifPresent(
                review -> reviewRepository.deleteById(
                        review.getId()));
    ...
    public List<Review> getMyReviews() {
        return getReviewsByUser(
                authService.requireCurrentUser().getId());
    }
    ...
    public List<Review> getReviewsForRestaurant(
            String restaurantId) {
     ...
     *
     * A deleted author falls back to the raw ID instead of hiding the review.
     */
    ...
    /**
     * Produces the next ID in the RV001 format, from the highest existing ID
     * rather than the number of reviews, so that a deleted review's ID is not
     * handed to a different one later.
     */
